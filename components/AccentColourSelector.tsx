...
import { useAccentColour } from "contexts/accent-colour-context";
...
const AccentColourSelector = ({ className }: AccentColourSelectorProps) => {
  const { colour, setColour } = useAccentColour();
    ...
    <Popover className={className + " relative h-6 w-6"}>
      <Popover.Button
        className={classNames(
          ...
          `transition-colors bg-accent-500 ring-accent-500`
        )}
